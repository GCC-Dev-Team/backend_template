 ...
 * @author I Nhrl
 */
public class WeJsapiPayService {
    /** 关闭订单 */
    public static void closeOrder(WeChatPayConfig payConfig, @NotBlank String outTradeNo) {
        JsapiServiceExtension service = new JsapiServiceExtension.Builder().config(payConfig.getWxPayConfig()).build();
    ...
    public static WePrepayResponseBO prepayWithRequestPayment(WeChatPayConfig payConfig, WePrepayUnifiedBO bo) {
        ...
        PrepayRequest request = bo.getJsapiPrepay();
        ...
        return new WePrepayResponseBO().setType(WePaymentsTypeEnum.JSAPI).setJsapiWith(paymentResponse);
    }
    /** 微信支付订单号查询订单 */
    public static WeQueryOrderResponseBO queryOrderById(WeChatPayConfig payConfig, @NotBlank String orderId) {
        ...
        return new WeQueryOrderResponseBO().setType(WePaymentsTypeEnum.JSAPI).setTransaction(transaction);
    }
    /** 商户订单号查询订单 */
